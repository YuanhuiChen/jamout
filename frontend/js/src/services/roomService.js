...
* @type {Array}
*/
jamout.services.RoomService.SOUNDLIST = [
...
*/
jamout.services.RoomService.prototype.preLoadSounds = function () {
      for (var i = 0; i < jamout.services.RoomService.SOUNDLIST.length; i++) {
        console.log('URL IS',jamout.services.RoomService.SOUNDLIST[i].URL);
        /** @const */
        var initSound = new Audio();
        initSound.src = jamout.services.RoomService.SOUNDLIST[i].URL;
        /** @const */
        initSound.load();
        if (initSound) {
            jamout.services.RoomService.SOUNDLIST[i].LOADED = initSound;
         } else {
            console.log('failed to load sound', jamout.services.RoomService.SOUNDLIST[i].NAME);
            console.log('todo: retry loading');
        }
      }
...
*/
jamout.services.RoomService.prototype.playSound = function(soundName) {
      /** @const */
      var name = soundName || "";

      for (var i = 0; i < jamout.services.RoomService.SOUNDLIST.length; i++) {
        if (name === jamout.services.RoomService.SOUNDLIST[i].NAME) {
          /** @const */
          var loadedSound = jamout.services.RoomService.SOUNDLIST[i].LOADED;
